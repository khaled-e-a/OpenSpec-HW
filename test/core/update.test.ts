...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    // Create synergyspec directory
    const openspecDir = path.join(testDir, 'synergyspec');
  ...
  describe('basic validation', () => {
    it('should throw error if synergyspec directory does not exist', async () => {
      // Remove synergyspec directory
      await fs.rm(path.join(testDir, 'synergyspec'), {
        recursive: true,
        force: true,
      });

      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No SynergySpec directory found. Run 'synergyspec-hw init' first."
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain('name: synergyspec-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'synergyspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'synergyspec-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('command updates', () => {
    it('should update synspec commands for configured Claude tool', async () => {
      ...
      // Check synspec command files were created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'synspec');
    ...
    it('should update core profile synspec commands when tool is configured', async () => {
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'synergyspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'synergyspec-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'synergyspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'synergyspec-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: synergyspec-explore');
      ...
      expect(cursorSkill).toContain('name: synergyspec-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'synergyspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'synergyspec-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: synspec-<id>.toml
      const qwenCmd = path.join(
        testDir,
        '.qwen',
        'commands',
        'synspec-explore.toml'
      );
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'synergyspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'synergyspec-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'synergyspec-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'synergyspec-apply-change', 'SKILL.md'),
        'old'
      );
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'synergyspec-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with SynergySpec markers
      const legacyContent = `${SYNERGYSPEC_MARKERS.start}
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new SynergySpec')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed SynergySpec markers from CLAUDE.md')
      );
...
`;
    ...
    it('should cleanup legacy slash command directories with --force', async () => {
      ...
      const legacyCommandDir = path.join(testDir, '.claude', 'commands', 'synergyspec');
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed .claude/commands/synergyspec/')
      );
    ...
    it('should cleanup legacy synergyspec/AGENTS.md with --force', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'synergyspec-explore'), {
        recursive: true,
      ...
      // Create legacy synergyspec/AGENTS.md
      await fs.writeFile(
        path.join(testDir, 'synergyspec', 'AGENTS.md'),
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed synergyspec/AGENTS.md')
      );
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new SynergySpec')
      );
    ...
    it('should remove SynergySpec marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'synergyspec-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + SynergySpec markers)
...
`;
      ...
      expect(updatedContent).not.toContain(SYNERGYSPEC_MARKERS.start);
      expect(updatedContent).not.toContain(SYNERGYSPEC_MARKERS.end);
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/synspec:new')
      );

      // Skills should be created
      const skillFile = path.join(testDir, '.claude', 'skills', 'synergyspec-explore', 'SKILL.md');
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, '.claude', 'commands', 'synergyspec', 'proposal.md'),
        'content'
      );

      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(
        path.join(testDir, '.cursor', 'commands', 'synergyspec-proposal.md'),
        'content'
      );
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'synergyspec-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'synergyspec-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'synergyspec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'synergyspec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'synergyspec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'synergyspec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'synergyspec-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New synspec commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'synspec');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'synergyspec-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'synergyspec-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'synergyspec-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'synergyspec-new-change', 'SKILL.md')
      )).toBe(true);
...
`
    ...
    it('should display extra workflows note when workflows outside profile exist', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'synergyspec-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'synergyspec-new-change', 'SKILL.md'), 'old');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'synergyspec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'synergyspec-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'synergyspec-hw init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'synergyspec-hw init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'synergyspec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'synergyspec-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'synergyspec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'synergyspec-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'synergyspec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'synergyspec-apply-change', 'SKILL.md'), 'content');

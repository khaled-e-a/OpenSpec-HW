...
describe('FishInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `synergyspec-fish-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return standard fish completions path', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'synergyspec-hw.fish'));
    });
    ...
    it('should use homeDir from constructor', () => {
      ...
      expect(result).toBe(path.join(customHome, '.config', 'fish', 'completions', 'synergyspec-hw.fish'));
    });
  });
  ...
  describe('install', () => {
    const mockCompletionScript = `# Fish completion script for SynergySpec CLI
function __fish_openspec
    echo "test"
end

complete -c synergyspec-hw -a 'init' -d 'Initialize SynergySpec'
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'synergyspec-hw.fish'));
    ...
    it('should create parent directories if they do not exist', async () => {
      ...
      expect(result.success).toBe(true);
      const targetPath = path.join(testHomeDir, '.config', 'fish', 'completions', 'synergyspec-hw.fish');
    ...
    it('should update when content is different', async () => {
      ...
      const updatedScript = `# Fish completion script for SynergySpec CLI
function __fish_synergyspec_new
    echo "updated"
end

complete -c synergyspec-hw -a 'init' -d 'Initialize SynergySpec'
complete -c synergyspec-hw -a 'validate' -d 'Validate specs'
`;
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `synergyspec fish test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('synergyspec fish test');
...
complete -c synergyspec-hw -a 'init'
`;

...
describe('PowerShellInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `synergyspec-powershell-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'SynergySpecCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'SynergySpecCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'SynergySpecCompletion.ps1'));
    });
  });
  ...
  describe('configureProfile', () => {
    const mockScriptPath = '/path/to/SynergySpecCompletion.ps1';

    // Note: SYNERGYSPEC_NO_AUTO_CONFIG check is now handled in the install() method,
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(content).not.toContain('# SynergySpec completions');
  ...
  describe('install', () => {
    const mockCompletionScript = `# PowerShell completion script for SynergySpec
$synergyspecHwCompleter = {
    param($wordToComplete, $commandAst, $cursorPosition)
    # Completion logic here
}
Register-ArgumentCompleter -CommandName synergyspec-hw -ScriptBlock $synergyspecHwCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('SynergySpecCompletion.ps1');
    ...
    // Note: SYNERGYSPEC_NO_AUTO_CONFIG support was removed from PowerShell installer
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `synergyspec powershell test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('synergyspec powershell test');
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# SYNERGYSPEC:START');
      ...
      expect(profileContentAfter).not.toContain('# SYNERGYSPEC:START');
    });

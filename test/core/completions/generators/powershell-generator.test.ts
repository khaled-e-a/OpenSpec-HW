...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for SynergySpec CLI');
			expect(script).toContain('$synergyspecHwCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for synergyspec-hw command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize SynergySpec',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName synergyspec-hw');
			expect(script).toContain('-ScriptBlock $synergyspecHwCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-SynergySpecChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-SynergySpecSpecs');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-SynergySpecChanges');
			expect(script).toContain('synergyspec-hw __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-SynergySpecSpecs');
			expect(script).toContain('synergyspec-hw __complete specs 2>$null');
		});

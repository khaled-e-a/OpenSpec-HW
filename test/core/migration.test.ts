...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `synergyspec-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `synergyspec-migration-config-${randomUUID()}`);
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'synergyspec-explore');
    await writeSkill(projectDir, 'synergyspec-apply-change');
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'synspec', 'my-custom.md');

...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `synergyspec-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing synergyspec/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No SynergySpec changes directory found. Run 'synergyspec-hw init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'synergyspec', 'changes');

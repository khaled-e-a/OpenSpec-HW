...
describe('FileSystemUtils.updateFileWithMarkers', () => {
  let testDir: string;
  const START_MARKER = '<!-- SYNERGYSPEC:START -->';
  const END_MARKER = '<!-- SYNERGYSPEC:END -->';

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `synergyspec-marker-test-${Date.now()}`);
  ...
  describe('new file creation', () => {
    it('should create new file with markers and content', async () => {
      ...
      const content = 'SynergySpec content';
  ...
  describe('existing file without markers', () => {
    it('should prepend markers and content to existing file', async () => {
      ...
      const newContent = 'SynergySpec content';
      await FileSystemUtils.updateFileWithMarkers(
        filePath,
        newContent,
        START_MARKER,
  ...
  describe('existing file with markers', () => {
    it('should replace content between markers', async () => {
      ...
      const oldManagedContent = 'Old SynergySpec content';
      const afterContent = '# After\nSome content after';
      
      const existingFile = `${beforeContent}\n${START_MARKER}\n${oldManagedContent}\n${END_MARKER}\n${afterContent}`;
      await fs.writeFile(filePath, existingFile);
      
      const newContent = 'New SynergySpec content';
      await FileSystemUtils.updateFileWithMarkers(
        filePath,
        newContent,
        START_MARKER,
...
After block content`;
  ...
  describe('shell markers', () => {
    const SHELL_START = '# SYNERGYSPEC:START';
    const SHELL_END = '# SYNERGYSPEC:END';
...
${SHELL_START}
# SynergySpec managed
alias synergyspec-hw="npx synergyspec-hw"
${SHELL_END}
...
export EDITOR="vim"`;
      ...
      expect(result).not.toContain('alias synergyspec-hw');

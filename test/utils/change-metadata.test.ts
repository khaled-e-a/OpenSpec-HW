...
describe('writeChangeMetadata', () => {
  ...
  beforeEach(async () => {
    ...
    changeDir = path.join(testDir, 'synergyspec', 'changes', 'test-change');
  ...
  it('should write valid YAML metadata file', async () => {
    ...
    const metaPath = path.join(changeDir, '.synergyspec.yaml');
...
describe('resolveSchemaForChange', () => {
  ...
  it('should use project config schema when no metadata exists', async () => {
    ...
    const configDir = path.join(testDir, 'synergyspec');

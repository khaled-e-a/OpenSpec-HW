...
const METADATA_FILENAME = '.synergyspec.yaml';
...
/**
 * Writes change metadata to .synergyspec.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .synergyspec.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .synergyspec.yaml metadata (if exists)
 * 3. Schema from synergyspec/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/synergyspec/changes/change-name)

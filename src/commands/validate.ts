...
export class ValidateCommand {
  ...
  private printNonInteractiveHint(): void {
    ...
    console.error('  synergyspec-hw validate --all');
    console.error('  synergyspec-hw validate --changes');
    console.error('  synergyspec-hw validate --specs');
    console.error('  synergyspec-hw validate <item-name>');
  ...
  private async validateDirectItem(itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    ...
    if (!opts.typeOverride && isChange && isSpec) {
      ...
      console.error('Pass --type change|spec, or use: synergyspec-hw change validate / openspec spec validate');
      process.exitCode = 1;
      return;
    }
  ...
  private async validateByType(type: ItemType, id: string, opts: { strict: boolean; json: boolean }): Promise<void> {
    ...
    if (type === 'change') {
      const changeDir = path.join(process.cwd(), 'synergyspec', 'changes', id);
    ...
    const file = path.join(process.cwd(), 'synergyspec', 'specs', id, 'spec.md');
    const start = Date.now();
  ...
  private printNextSteps(type: ItemType): void {
    ...
    if (type === 'change') {
      ...
      bullets.push('- Debug parsed deltas: synergyspec-hw change show <id> --json --deltas-only');
    } else {
  ...
  private async runBulkValidation(scope: { changes: boolean; specs: boolean }, opts: { strict: boolean; json: boolean; concurrency?: string; noInteractive?: boolean }): Promise<void> {
    ...
    const concurrency = normalizeConcurrency(opts.concurrency) ?? normalizeConcurrency(process.env.SYNERGYSPEC_CONCURRENCY) ?? DEFAULT_CONCURRENCY;
    const validator = new Validator(opts.strict);

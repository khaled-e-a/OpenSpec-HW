...
import { SYNERGYSPEC_DIR_NAME } from '../core/config.js';
...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  onboard: {
    name: 'Onboard',
    description: 'Guided onboarding flow for SynergySpec',
  },
};
...
function maybeWarnConfigDrift(
  ...
  const openspecDir = path.join(projectDir, SYNERGYSPEC_DIR_NAME);
  ...
  console.log(colorize('Warning: Global config is not applied to this project. Run `synergyspec-hw update` to sync.'));
}
...
export function registerConfigCommand(program: Command): void {
  const configCmd = program
    .command('config')
    .description('View and modify global SynergySpec configuration')
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "synergyspec-hw config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: synergyspec-hw config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `synergyspec-hw config profile core`
      if (preset === 'all') {
        const config = getGlobalConfig();
        config.profile = 'all';
        ...
        console.log('Config updated. Run `synergyspec-hw update` in your projects to apply.');
        return;
      }
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `synergyspec-hw config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        // Check if inside an SynergySpec project
        ...
        if (fs.existsSync(openspecDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx synergyspec-hw update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `synergyspec-hw update` in your other projects to apply.');
            } catch {
              console.error('`synergyspec-hw update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }

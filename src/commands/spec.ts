...
const SPECS_DIR = 'synergyspec/specs';
...
function filterSpec(spec: Spec, options: ShowOptions): Spec {
  ...
  const metadata = spec.metadata ?? { version: '1.0.0', format: 'synergyspec' as const };
...
export class SpecCommand {
  private SPECS_DIR = 'synergyspec/specs';
  ...
  async show(specId?: string, options: ShowOptions = {}): Promise<void> {
    ...
    if (!existsSync(specPath)) {
      throw new Error(`Spec '${specId}' not found at synergyspec/specs/${specId}/spec.md`);
    }
    ...
    if (options.json) {
      ...
      const output = {
        ...
        metadata: parsed.metadata ?? { version: '1.0.0', format: 'synergyspec' as const },
      };
...
export function registerSpecCommand(rootProgram: typeof program) {
  ...
  specCommand.hook('preAction', () => {
    console.error('Warning: The "synergyspec-hw spec ..." commands are deprecated. Prefer verb-first commands (e.g., "synergyspec-hw show", "synergyspec-hw validate --specs").');
  });

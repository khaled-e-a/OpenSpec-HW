/**
 * Legacy cleanup module for detecting and removing SynergySpec artifacts
...
import { SYNERGYSPEC_MARKERS } from './config.js';

/**
 * Legacy config file names from the old ToolRegistry.
 * These were config files created at project root with SynergySpec markers.
 */
export const LEGACY_CONFIG_FILES = [
  ...
  'QODER.md',
  'IFLOW.md',
  'AGENTS.md', // root AGENTS.md (not synergyspec/AGENTS.md)
  'QWEN.md',
] as const;
...
export const LEGACY_SLASH_COMMAND_PATHS: Record<string, LegacySlashCommandPattern> = {
  // Directory-based: .tooldir/commands/synergyspec/ (Gen 1 — very old upstream format)
  'claude': { type: 'directory', path: '.claude/commands/synspec' },
  'codebuddy': { type: 'directory', path: '.codebuddy/commands/synspec' },
  'qoder': { type: 'directory', path: '.qoder/commands/synspec' },
  'crush': { type: 'directory', path: '.crush/commands/synspec' },
  'gemini': { type: 'directory', path: '.gemini/commands/synspec' },
  'costrict': { type: 'directory', path: '.cospec/synergyspec/commands' },

  // File-based: individual synergyspec-*.md files in a commands/workflows/prompts folder
  'cursor': { type: 'files', pattern: '.cursor/commands/synergyspec-*.md' },
  'windsurf': { type: 'files', pattern: '.windsurf/workflows/synergyspec-*.md' },
  'kilocode': { type: 'files', pattern: '.kilocode/workflows/synergyspec-*.md' },
  'kiro': { type: 'files', pattern: '.kiro/prompts/synergyspec-*.prompt.md' },
  'github-copilot': { type: 'files', pattern: '.github/prompts/synergyspec-*.prompt.md' },
  'amazon-q': { type: 'files', pattern: '.amazonq/prompts/synergyspec-*.md' },
  'cline': { type: 'files', pattern: '.clinerules/workflows/synergyspec-*.md' },
  'roocode': { type: 'files', pattern: '.roo/commands/synergyspec-*.md' },
  'auggie': { type: 'files', pattern: '.augment/commands/synergyspec-*.md' },
  'factory': { type: 'files', pattern: '.factory/commands/synergyspec-*.md' },
  'opencode': { type: 'files', pattern: '.opencode/command/synergyspec-*.md' },
  'continue': { type: 'files', pattern: '.continue/prompts/synergyspec-*.prompt' },
  'antigravity': { type: 'files', pattern: '.agent/workflows/synergyspec-*.md' },
  'iflow': { type: 'files', pattern: '.iflow/commands/synergyspec-*.md' },
  'qwen': { type: 'files', pattern: '.qwen/commands/synergyspec-*.toml' },
  'codex': { type: 'files', pattern: '.codex/prompts/synergyspec-*.md' },
};
...
export interface LegacyDetectionResult {
  /** Config files with SynergySpec markers detected */
  configFiles: string[];
  ...
  /** Whether synergyspec/AGENTS.md exists */
  hasOpenspecAgents: boolean;
  /** Whether synergyspec/project.md exists (preserved, migration hint only) */
  hasProjectMd: boolean;
  /** Whether root AGENTS.md has SynergySpec markers */
...
/**
 * Detects all legacy SynergySpec artifacts in a project.
 *
...
/**
 * Detects legacy config files with SynergySpec markers.
...
export async function detectLegacyConfigFiles(
  ...
  for (const fileName of LEGACY_CONFIG_FILES) {
    ...
    if (await FileSystemUtils.fileExists(filePath)) {
      ...
      if (hasSynergySpecMarkers(content)) {
 ...
 *
 * @param projectPath - The root path of the project
 * @param pattern - Glob pattern like '.cursor/commands/synergyspec-*.md'
...
async function findLegacySlashCommandFiles(
  ...
  try {
    ...
    // synergyspec-*.md -> /^synergyspec-.*\.md$/
    // synergyspec-*.prompt.md -> /^synergyspec-.*\.prompt\.md$/
    // synergyspec-*.toml -> /^synergyspec-.*\.toml$/
...
/**
 * Detects legacy SynergySpec structure files (AGENTS.md and project.md).
 *
 * @param projectPath - The root path of the project
...
export async function detectLegacyStructureFiles(
  ...
  // Check for synergyspec/AGENTS.md
  const openspecAgentsPath = FileSystemUtils.joinPath(projectPath, 'synergyspec', 'AGENTS.md');
  hasOpenspecAgents = await FileSystemUtils.fileExists(openspecAgentsPath);

  // Check for synergyspec/project.md (for migration messaging, not deleted)
  const projectMdPath = FileSystemUtils.joinPath(projectPath, 'synergyspec', 'project.md');
  hasProjectMd = await FileSystemUtils.fileExists(projectMdPath);

  // Check for root AGENTS.md with SynergySpec markers
  ...
  if (await FileSystemUtils.fileExists(rootAgentsPath)) {
    ...
    hasRootAgentsWithMarkers = hasSynergySpecMarkers(content);
  }
...
/**
 * Checks if content contains SynergySpec markers.
 *
 * @param content - File content to check
 * @returns True if both start and end markers are present
 */
export function hasSynergySpecMarkers(content: string): boolean {
  return (
    content.includes(SYNERGYSPEC_MARKERS.start) && content.includes(SYNERGYSPEC_MARKERS.end)
  );
}

/**
 * Checks if file content is 100% SynergySpec content (only markers and whitespace outside).
 *
 * @param content - File content to check
 * @returns True if content outside markers is only whitespace
 */
export function isOnlySynergySpecContent(content: string): boolean {
  const startIndex = content.indexOf(SYNERGYSPEC_MARKERS.start);
  const endIndex = content.indexOf(SYNERGYSPEC_MARKERS.end);
  ...
  const after = content.substring(endIndex + SYNERGYSPEC_MARKERS.end.length);

  return before.trim() === '' && after.trim() === '';
}

/**
 * Removes the SynergySpec marker block from file content.
 ...
 *
 * @param content - File content with SynergySpec markers
 * @returns Content with marker block removed
 */
export function removeMarkerBlock(content: string): string {
  return removeMarkerBlockUtil(content, SYNERGYSPEC_MARKERS.start, SYNERGYSPEC_MARKERS.end);
}
...
/**
 * Cleans up legacy SynergySpec artifacts from a project.
 * Preserves synergyspec/project.md (shows migration hint instead of deleting).
 *
 * @param projectPath - The root path of the project
...
export async function cleanupLegacyArtifacts(
  ...
  // Delete legacy slash command directories (these are 100% SynergySpec-managed)
  ...
  // Delete legacy slash command files (these are 100% SynergySpec-managed)
  ...
  // Delete synergyspec/AGENTS.md (this is inside openspec/, it's SynergySpec-managed)
  if (detection.hasOpenspecAgents) {
    const agentsPath = FileSystemUtils.joinPath(projectPath, 'synergyspec', 'AGENTS.md');
    if (await FileSystemUtils.fileExists(agentsPath)) {
      try {
        await fs.unlink(agentsPath);
        result.deletedFiles.push('synergyspec/AGENTS.md');
      } catch (error: any) {
        result.errors.push(`Failed to delete synergyspec/AGENTS.md: ${error.message}`);
      }
    }
  }

  // Handle root AGENTS.md with SynergySpec markers - remove markers only, NEVER delete
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const dir of result.deletedDirs) {
      lines.push(`  ✓ Removed ${dir}/ (replaced by /synspec:*)`);
    }

    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ Removed SynergySpec markers from ${file}`);
    }
  }
 ...
 * Only includes SynergySpec-managed files (slash commands, synergyspec/AGENTS.md).
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  // Slash command directories (these are 100% SynergySpec-managed)
  ...
  // Slash command files (these are 100% SynergySpec-managed)
  for (const file of detection.slashCommandFiles) {
    removals.push({ path: file, explanation: 'replaced by skills/' });
  }

  // synergyspec/AGENTS.md (inside openspec/, it's SynergySpec-managed)
  if (detection.hasOpenspecAgents) {
    removals.push({ path: 'synergyspec/AGENTS.md', explanation: 'obsolete workflow file' });
  }
...
function buildUpdatesList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const file of detection.configFilesToUpdate) {
    updates.push({ path: file, explanation: 'removing SynergySpec markers' });
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push(chalk.bold('Upgrading to the new SynergySpec'));
  lines.push('');
  lines.push('SynergySpec now uses agent skills, the emerging standard across coding');
  ...
  if (updates.length > 0) {
    ...
    lines.push(chalk.dim('SynergySpec markers will be removed, your content preserved:'));
  ...
  for (const file of detection.slashCommandFiles) {
    ...
    for (const [toolId, pattern] of Object.entries(LEGACY_SLASH_COMMAND_PATHS)) {
      if (pattern.type === 'files' && pattern.pattern) {
        ...
        // e.g., '.cursor/commands/synergyspec-*.md' -> /^\.cursor\/commands\/synergyspec-.*\.md$/
...
export function formatProjectMdMigrationHint(): string {
  ...
  lines.push('  • synergyspec/project.md');
  lines.push(chalk.dim('    We won\'t delete this file. It may contain useful project context.'));
  lines.push('');
  lines.push(chalk.dim('    The new synergyspec/config.yaml has a "context:" section for planning'));
  lines.push(chalk.dim('    context. This is included in every SynergySpec request and works more'));

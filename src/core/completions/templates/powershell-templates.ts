...
function Get-SynergySpecChanges {
    $output = synergyspec-hw __complete changes 2>$null
    if ($output) {
...
function Get-SynergySpecSpecs {
    $output = synergyspec-hw __complete specs 2>$null
    if ($output) {
        $output | ForEach-Object {
            ($_ -split "\\t")[0]
        }

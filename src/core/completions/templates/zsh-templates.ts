...
# Use synergyspec-hw __complete to get available changes
_synergyspec_hw_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(synergyspec-hw __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use synergyspec-hw __complete to get available specs
_synergyspec_hw_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(synergyspec-hw __complete specs 2>/dev/null)

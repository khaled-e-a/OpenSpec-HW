 ...
 * Follows Zsh completion system conventions using the _openspec_hw function.
 */
export class ZshGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      commandCaseLines.push(`          _openspec_hw_${this.sanitizeFunctionName(cmd.name)}`);
...
${commandList}
  ...
  case $state in
    command)
      _describe "openspec-hw command" commands
      ;;
    args)
      case $words[1] in
${commandCases}
...
${commandFunctions}
${helpers}
compdef _openspec_hw openspec-hw
`;
  }
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    const funcName = `_openspec_hw_${this.sanitizeFunctionName(cmd.name)}`;
    const lines: string[] = [];
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(`          _openspec_hw_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`);
        lines.push('          ;;');
      }
  ...
  private generateSubcommandFunction(parentName: string, subcmd: CommandDefinition): string[] {
    const funcName = `_openspec_hw_${this.sanitizeFunctionName(parentName)}_${this.sanitizeFunctionName(subcmd.name)}`;
    const lines: string[] = [];
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      case 'change-id':
        return "'*: :_openspec_hw_complete_changes'";
      case 'spec-id':
        return "'*: :_openspec_hw_complete_specs'";
      case 'change-or-spec-id':
        return "'*: :_openspec_hw_complete_items'";
      case 'path':
        return "'*:path:_files'";
      case 'shell':

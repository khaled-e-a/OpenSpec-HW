 ...
 *
 * @param projectRoot - The root directory of the project (where `synergyspec/` lives)
 * @returns Parsed config or null if file doesn't exist
 */
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  // Try both .yaml and .yml, prefer .yaml
  let configPath = path.join(projectRoot, 'synergyspec', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'synergyspec', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`synergyspec/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse synergyspec/config.yaml:`, error);
    return null;
  }
}

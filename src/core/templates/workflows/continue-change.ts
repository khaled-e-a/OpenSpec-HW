...
export function getContinueChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'synergyspec-continue-change',
    description: 'Continue working on an SynergySpec change by creating the next artifact. Use when the user wants to progress their change, create the next artifact, or continue their workflow.',
...
3. **Act based on status**:
   ...
   **If all artifacts are complete (\`isComplete: true\`)**:
   - Congratulate the user
   - Show final status including the schema used
   - Suggest: "All artifacts created! Implement this change with \`/synspec:apply\` (or \`/synspec:tdd\` for red-green-refactor TDD), or archive it with \`/synspec:archive\`."
   - STOP
...
- Current progress (N/M complete)
- What artifacts are now unlocked
- Prompt: "Run \`/synspec:continue\` to create the next artifact"

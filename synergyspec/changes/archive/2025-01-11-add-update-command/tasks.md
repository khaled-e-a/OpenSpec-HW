...
- [x] 1.3 Write `readmeTemplate` to `synergyspec/README.md` using `FileSystemUtils.writeFile`
- [x] 1.4 Update `CLAUDE.md` using markers via `FileSystemUtils.updateFileWithMarkers` and `TemplateManager.getClaudeTemplate()`
- [x] 1.5 Display ASCII-safe success message: `Updated SynergySpec instructions`

## 2. CLI Integration
- [x] 2.1 Register `update` command in `src/cli/index.ts`
- [x] 2.2 Add command description: `Update SynergySpec instruction files`
- [x] 2.3 Handle errors with `ora().fail(...)` and exit code 1 (missing `openspec` directory, file write errors)

## 3. Testing
- [x] 3.1 Verify `synergyspec/README.md` is fully replaced with latest template
- [x] 3.2 Verify `CLAUDE.md` SynergySpec block updates without altering user content outside markers

...
SynergySpec users on NixOS or using the Nix package manager cannot easily install or run SynergySpec without going through npm. Adding a Nix flake makes SynergySpec a first-class citizen in the Nix ecosystem, enabling users to run `nix run github:Fission-AI/SynergySpec -- init` or include SynergySpec in their development environments declaratively.
...
### New Capabilities
- `nix-flake-support`: Nix flake configuration for building and running SynergySpec

...
SynergySpec currently has no visibility into how the tool is being used. Without analytics, we cannot:
...
- Store anonymous ID in global config (`~/.config/synergyspec/config.json`)
- Respect `DO_NOT_TRACK` and `SYNERGYSPEC_TELEMETRY=0` environment variables

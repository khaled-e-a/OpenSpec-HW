...
## Summary
- Enable SynergySpec to generate and update custom slash commands for supported coding agents (Claude Code and Cursor).
- Provide three slash commands aligned with SynergySpec's workflow: proposal (start a change proposal), apply (implement), and archive.
- Share slash command templating between agents to make future extensions simple.

## Motivation
Developers use different coding agents and editors. Having consistent slash commands across tools for the SynergySpec workflow reduces friction and ensures a standard way to trigger the workflow. Supporting both Claude Code and Cursor now lays a foundation for future agents that introduce slash command features.

## Proposal
1. During `openspec init`, when a user selects a supported tool, generate slash command configuration for three SynergySpec workflow stages:
   - Claude (namespaced): `/synergyspec/proposal`, `/synergyspec/apply`, `/synergyspec/archive`.
   - Cursor (flat, prefixed): `/synergyspec-proposal`, `/synergyspec-apply`, `/synergyspec-archive`.
   - Semantics:
   ...
   - Each command file MUST embed concise, step-by-step instructions sourced from `synergyspec/README.md` (see Template Content section).
2. Store slash command files per tool:
   - Claude Code: `.claude/commands/synergyspec/{proposal,apply,archive}.md`
   - Cursor: `.cursor/commands/{synergyspec-proposal,synergyspec-apply,synergyspec-archive}.md`
...
3. Command file format and metadata:
   ...
   - Place SynergySpec markers around the body only, never inside frontmatter.
   - Keep the visible slash name, file name, and any frontmatter `name`/`id` consistently aligned (e.g., `proposal`, `synergyspec-proposal`).
   - Namespacing: categorize these under “SynergySpec” and prefer unique IDs (e.g., `synergyspec-proposal`) to avoid collisions.
...
- Templates live in `TemplateManager` with helpers that extract concise, authoritative snippets from `synergyspec/README.md`.
...
- Directory creation for `.claude/commands/synergyspec/` and `.cursor/commands/` is the configurator’s responsibility.

### Command Naming & UX
- Claude Code: use namespacing in the slash itself for readability and grouping: `/synergyspec/proposal`, `/synergyspec/apply`, `/synergyspec/archive`.
- Cursor: use flat names with an `synergyspec-` prefix: `/synergyspec-proposal`, `/synergyspec-apply`, `/synergyspec-archive`. Group via `category: SynergySpec` when supported.
- Consistency: align file names, visible slash names, and any frontmatter `id` (e.g., `id: synergyspec-apply`).
...
### Claude Code: `.claude/commands/synergyspec/proposal.md`
```markdown
---
name: SynergySpec: Proposal
description: Scaffold a new SynergySpec change and validate strictly.
category: SynergySpec
...
Slash invocation: `/synergyspec/proposal` (namespaced)

### Cursor: `.cursor/commands/synergyspec-proposal.md`
```markdown
---
name: /synergyspec-proposal
id: synergyspec-proposal
category: SynergySpec
description: Scaffold a new SynergySpec change and validate strictly.
---
<!-- OPENSPEC:START -->
...
Slash invocation: `/synergyspec-proposal` (flat, prefixed)

## Template Content
Templates should be brief, actionable, and sourced from `synergyspec/README.md` to avoid duplication. Each command body includes:

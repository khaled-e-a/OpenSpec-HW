...
- **THEN** create `.gemini/commands/synergyspec/proposal.toml`, `.gemini/commands/synergyspec/apply.toml`, and `.gemini/commands/synergyspec/archive.toml`
- **AND** populate each file as TOML that sets a stage-specific `description = "<summary>"` and a multi-line `prompt = """` block with the shared SynergySpec template
- **AND** wrap the SynergySpec managed markers (`<!-- OPENSPEC:START -->` / `<!-- OPENSPEC:END -->`) inside the `prompt` value so `openspec update` can safely refresh the body between markers without touching the TOML framing
...
- **THEN** create `.iflow/commands/synergyspec-proposal.md`, `.iflow/commands/synergyspec-apply.md`, and `.iflow/commands/synergyspec-archive.md`
- **AND** populate each file from shared templates so command text matches other tools
...
- **THEN** create `.roo/commands/synergyspec-proposal.md`, `.roo/commands/synergyspec-apply.md`, and `.roo/commands/synergyspec-archive.md`
- **AND** populate each file from shared templates so command text matches other tools
- **AND** include simple Markdown headings (e.g., `# SynergySpec: Proposal`) without YAML frontmatter
- **AND** wrap the generated content in SynergySpec managed markers where applicable so `openspec update` can safely refresh the commands
- **AND** each template includes instructions for the relevant SynergySpec workflow stage

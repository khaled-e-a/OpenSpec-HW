...
The templates in `src/core/templates/skill-templates.ts` use Claude's colon-based format (`/synspec:new`) as the canonical format. Tools that use different formats need transformation at generation time.

## Goals / Non-Goals

**Goals:**
- Transform all `/synspec:` command references to `/synspec-` for OpenCode in both commands and skills
...
**Non-Goals:**
- Modifying the canonical template format (templates stay with `/synspec:`)
...
| Risk | Mitigation |
|------|------------|
| Other `/synspec:` patterns exist that shouldn't be transformed | All occurrences in templates are command invocations - verified by inspection |

...
state via `process.cwd()` and `synergyspec/changes`. This makes outcomes depend on
...
- Each suite that touches filesystem discovery creates a temporary directory:
  - `synergyspec/changes/sample-change/proposal.md`
  - `synergyspec/changes/sample-change/specs/sample/spec.md`
...
- Tests that previously depended on repo state now:
  ...
  - Do not read real `synergyspec/changes` during execution.

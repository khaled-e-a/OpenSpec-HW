...
- Teams already rely on SynergySpec to manage slash-command configurations for Claude Code, Cursor, OpenCode, Codex, Kilo Code, and Windsurf. Excluding GitHub Copilot forces developers to manually maintain SynergySpec prompts in `.github/prompts/`, which leads to drift and undermines SynergySpec's "single source of truth" promise.
- GitHub Copilot discovers prompts from the repository's `.github/prompts/` directory, making it straightforward to version control and share across the team. Adding automated generation and refresh through `openspec init` and `openspec update` eliminates manual synchronization and keeps SynergySpec instructions consistent across all AI assistants.

## What Changes
- Add GitHub Copilot to the `openspec init` tool picker with "already configured" detection similar to other editors, wiring an implementation that writes managed Markdown prompt files to `.github/prompts/` with SynergySpec marker blocks.
- Generate three GitHub Copilot prompt files—`synergyspec-proposal.prompt.md`, `synergyspec-apply.prompt.md`, and `synergyspec-archive.prompt.md`—whose content mirrors shared slash-command templates while conforming to Copilot's frontmatter and `$ARGUMENTS` placeholder convention.
- Document GitHub Copilot's repository-based discovery and that SynergySpec writes prompts to `.github/prompts/` with managed blocks.

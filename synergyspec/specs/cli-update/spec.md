...
As a developer using SynergySpec, I want to update the SynergySpec instructions in my project when new versions are released, so that I can benefit from improvements to AI agent instructions.
## Requirements
### Requirement: Update Behavior
The update command SHALL update SynergySpec instruction files to the latest templates in a team-friendly manner.

#### Scenario: Running update command
- **WHEN** a user runs `openspec update`
- **THEN** replace `synergyspec/AGENTS.md` with the latest template
- **AND** if a root-level stub (`AGENTS.md`/`CLAUDE.md`) exists, refresh it so it points to `@/synergyspec/AGENTS.md`

### Requirement: Prerequisites

The command SHALL require an existing SynergySpec structure before allowing updates.
...
- **THEN** display error: "No SynergySpec directory found. Run 'openspec init' first."
...
- **WHEN** updating files
- **THEN** completely replace `synergyspec/AGENTS.md` with the latest template
- **AND** if a root-level stub exists, update the managed block content so it keeps directing teammates to `@/synergyspec/AGENTS.md`

### Requirement: Tool-Agnostic Updates
The update command SHALL refresh SynergySpec-managed files in a predictable manner while respecting each team's chosen tooling.
...
- **AND** update only the SynergySpec-managed sections inside existing AI tool files, leaving user-authored content untouched
...
- **THEN** replace `synergyspec/AGENTS.md` with the latest template
- **AND** if a root-level stub exists, refresh it so it still directs contributors to `@/synergyspec/AGENTS.md`
...
- **WHEN** `.agent/workflows/` contains `synergyspec-proposal.md`, `synergyspec-apply.md`, and `synergyspec-archive.md`
- **THEN** refresh the SynergySpec-managed portion of each file so the workflow copy matches other tools while preserving the existing single-field `description` frontmatter
...
- **WHEN** `.codebuddy/commands/synergyspec/` contains `proposal.md`, `apply.md`, and `archive.md`
...
- **AND** preserve any user customizations outside the SynergySpec managed markers

#### Scenario: Updating slash commands for Cline
- **WHEN** `.clinerules/workflows/` contains `synergyspec-proposal.md`, `synergyspec-apply.md`, and `synergyspec-archive.md`
- **THEN** refresh each file using shared templates
- **AND** include Cline-specific Markdown heading frontmatter
- **AND** ensure templates include instructions for the relevant workflow stage

#### Scenario: Updating slash commands for Continue
- **WHEN** `.continue/prompts/` contains `synergyspec-proposal.prompt`, `synergyspec-apply.prompt`, and `synergyspec-archive.prompt`
- **THEN** refresh each file using shared templates
- **AND** ensure templates include instructions for the relevant workflow stage

#### Scenario: Updating slash commands for Crush
- **WHEN** `.crush/commands/` contains `synergyspec/proposal.md`, `synergyspec/apply.md`, and `synergyspec/archive.md`
- **THEN** refresh each file using shared templates
...
- **WHEN** `.factory/commands/` contains `synergyspec-proposal.md`, `synergyspec-apply.md`, and `synergyspec-archive.md`
- **THEN** refresh each file using the shared Factory templates that include YAML frontmatter for the `description` and `argument-hint` fields
- **AND** ensure the template body retains the `$ARGUMENTS` placeholder so user input keeps flowing into droid
- **AND** update only the content inside the SynergySpec managed markers, leaving any unmanaged notes untouched
...
- **WHEN** `.kilocode/workflows/` contains `synergyspec-proposal.md`, `synergyspec-apply.md`, and `synergyspec-archive.md`
- **THEN** refresh each file using shared templates wrapped in SynergySpec markers
- **AND** ensure templates include instructions for the relevant workflow stage
- **AND** skip creating missing files (the update command only refreshes what already exists)

#### Scenario: Updating slash commands for Codex
- **GIVEN** the global Codex prompt directory contains `synergyspec-proposal.md`, `synergyspec-apply.md`, and `synergyspec-archive.md`
- **WHEN** a user runs `openspec update`
- **THEN** refresh each file using the shared slash-command templates (including placeholder guidance)
- **AND** preserve any unmanaged content outside the SynergySpec marker block
- **AND** skip creation when a Codex prompt file is missing

#### Scenario: Updating slash commands for GitHub Copilot
- **WHEN** `.github/prompts/` contains `synergyspec-proposal.prompt.md`, `synergyspec-apply.prompt.md`, and `synergyspec-archive.prompt.md`
- **THEN** refresh each file using shared templates while preserving the YAML frontmatter
- **AND** update only the SynergySpec-managed block between markers
- **AND** ensure templates include instructions for the relevant workflow stage

#### Scenario: Updating slash commands for Gemini CLI
- **WHEN** `.gemini/commands/synergyspec/` contains `proposal.toml`, `apply.toml`, and `archive.toml`
...
- **WHEN** `.iflow/commands/` contains `synergyspec-proposal.md`, `synergyspec-apply.md`, and `synergyspec-archive.md`
- **THEN** refresh each file using shared templates
...
- Update SynergySpec instructions with a single command

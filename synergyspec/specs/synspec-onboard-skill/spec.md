# synspec-onboard-skill Specification

## Purpose
Define `/synspec:onboard` behavior for guiding users through an end-to-end SynergySpec workflow on their real codebase.

## Requirements
### Requirement: OPSX Onboard Skill

The system SHALL provide an `/synspec:onboard` skill that guides users through their first complete SynergySpec workflow cycle with narration and real codebase work.

#### Scenario: Skill invocation

- **WHEN** user invokes `/synspec:onboard`
- **THEN** agent checks if SynergySpec is initialized
...
- **THEN** agent briefly demonstrates `/synspec:explore` by investigating relevant code
...
- **AND** provides command reference table (/synspec:explore, /synspec:new, /synspec:ff, /synspec:continue, /synspec:apply, /synspec:verify, /synspec:archive)
- **AND** suggests next actions (try /synspec:new or /synspec:ff on something)
...
- **AND** explains how to continue later with `/synspec:continue <name>`
...
- **AND** exits gracefully with encouragement to try `/synspec:new`

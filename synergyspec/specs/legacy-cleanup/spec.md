...
## Purpose
Define detection and cleanup behavior for legacy SynergySpec artifacts during initialization and update workflows.

## Requirements
### Requirement: Legacy artifact detection

The system SHALL detect legacy SynergySpec artifacts from previous init versions.

#### Scenario: Detecting legacy config files

- **WHEN** running `openspec init` on an existing project
- **THEN** the system SHALL check for config files with SynergySpec markers:
  - `CLAUDE.md`
  - `.cursorrules`
  - `.windsurfrules`
  - `.clinerules`
...
- **THEN** the system SHALL check for old slash command directories:
  ...
  - `.cursor/commands/synergyspec/` (note: old format used `synergyspec-*.md` in commands root)
  - `.windsurf/workflows/synergyspec-*.md`
  - And equivalent directories for all tools in the legacy SlashCommandRegistry

#### Scenario: Detecting legacy SynergySpec structure files

- **WHEN** running `openspec init` on an existing project
- **THEN** the system SHALL check for:
  - `synergyspec/AGENTS.md`
  - `synergyspec/project.md` (for migration messaging only, not deleted)
  - Root `AGENTS.md` with SynergySpec markers
...
The system SHALL preserve user content when removing SynergySpec markers from config files.

#### Scenario: Config file with only SynergySpec content

- **WHEN** a config file contains only SynergySpec marker block (whitespace outside is acceptable)
- **THEN** the system SHALL remove the SynergySpec marker block
...
- **WHEN** a config file contains content outside SynergySpec markers
...
- **WHEN** root `AGENTS.md` contains SynergySpec markers AND other content
- **THEN** the system SHALL remove only the SynergySpec marker block
...
- **WHEN** a legacy slash command directory exists (e.g., `.claude/commands/synergyspec/`)
...
- **WHEN** `synergyspec/AGENTS.md` exists
- **THEN** the system SHALL delete the file
- **AND** NOT delete the `synergyspec/` directory itself
...
- **THEN** users can migrate manually or use `/synspec:explore` to get AI assistance
...
- **THEN** the system SHALL display a summary section:
  ...
  Cleaned up legacy files:
    ✓ Removed SynergySpec markers from CLAUDE.md
    ✓ Removed .claude/commands/synergyspec/ (replaced by /synspec:*)
    ✓ Removed synergyspec/AGENTS.md (no longer needed)
  ```
- **AND IF** `synergyspec/project.md` exists

...
## Purpose
Define tool-agnostic command content and adapter contracts for generating tool-specific SynergySpec command files.
...
- **THEN** `CommandContent` SHALL include:
  ...
  - `name`: human-readable name (e.g., 'SynergySpec Explore')
  - `description`: brief description of command purpose
  - `category`: grouping category (e.g., 'SynergySpec')
...
- **THEN** the adapter SHALL output YAML frontmatter with `name` as `/synspec-<id>`, `id`, `category`, `description` fields
- **AND** file path SHALL follow pattern `.cursor/commands/synspec-<id>.md`

#### Scenario: Windsurf adapter formatting

- **WHEN** formatting a command for Windsurf
- **THEN** the adapter SHALL output YAML frontmatter with `name`, `description`, `category`, `tags` fields
- **AND** file path SHALL follow pattern `.windsurf/workflows/synspec-<id>.md`

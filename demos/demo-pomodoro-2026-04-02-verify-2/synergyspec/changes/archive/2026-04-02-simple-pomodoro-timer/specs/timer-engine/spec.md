...
## Overview
This spec implements requirements for the `timer-engine` capability — the core countdown timer logic that drives all session types. It handles starting, pausing, resuming, and resetting the countdown, as well as detecting and signalling session completion.
...
- **THEN** the countdown begins from the session's configured duration (e.g., 1500 s for Work)
...
**Implements**: UC1-S4 - User works uninterrupted while the timer counts down

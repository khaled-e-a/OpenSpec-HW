...
Defines the widget type registry system: how widget types are registered, how registered components are resolved and rendered inside the widget shell, error handling for unknown types, default size application on add, and the structure of the widget shell itself.

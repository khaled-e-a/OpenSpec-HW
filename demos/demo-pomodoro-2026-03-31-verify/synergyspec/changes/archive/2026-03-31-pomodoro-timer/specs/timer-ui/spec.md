...
## Overview
This spec implements requirements for the `timer-ui` capability — the user interface that displays the timer state and provides controls to interact with it.
...
- **THEN** the displayed time reflects the current remainingSeconds, formatted as zero-padded MM:SS (e.g., "24:59", "05:00", "00:01")

#### Scenario: Display at full duration
- **WHEN** the timer is in IDLE state
- **THEN** the display shows the full default duration for the current mode ("25:00" for work, "05:00" for rest)

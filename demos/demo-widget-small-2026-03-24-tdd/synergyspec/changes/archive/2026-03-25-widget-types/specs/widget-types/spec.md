...
This spec defines requirements for the widget-types capability: four typed widget content renderers (clock, image, file, webpage) each with an optional inline configuration panel. All content state is local to the React session; no persistence is required.

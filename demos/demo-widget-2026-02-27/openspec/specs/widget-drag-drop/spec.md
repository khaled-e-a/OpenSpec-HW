...
Defines the behaviour of widget drag-and-drop repositioning within the dashboard grid: how a drag is initiated via a handle, how a ghost element tracks the snapped target position, how the pointer is clamped to grid boundaries, how a drop commits or rejects the new position, and how Escape cancels an in-progress drag.

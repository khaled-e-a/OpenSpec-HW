...
Defines the widget type registry: how widget components are registered and resolved by type key, how invalid registration arguments are rejected, how duplicate keys are handled, and how the dashboard grid resolves and renders components (including fallback error tiles for unresolvable types) at render time.
